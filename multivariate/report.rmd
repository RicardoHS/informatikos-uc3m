...
We can then compare the silhouettes for both the $k=2$ and $k=3$ methods
...
Overall the best performing cluster method appears to be k-means clustering, but there are possibly two valid choices for $k$, $k=2$ and $k=3$. The cluster assignments for these two are respectively:

```{r, fig.height = 3}
p1 <- as_tibble(pca_poke$x[,1:2]) %>% 
  bind_cols(cluster = factor(kmeans_poke$cluster)) %>% 
  ggplot(aes(PC1,PC2, color = cluster))+
  geom_point()+
  theme(legend.position = "bottom")
p2 <- as_tibble(pca_poke$x[,1:2]) %>% 
  bind_cols(cluster = factor(kmeans_poke3$cluster)) %>% 
  ggplot(aes(PC1,PC2, color = cluster))+
  geom_point()+
  theme(legend.position = "bottom")
...
In both cases the clusters are not very well separated, but this is because the plot of the first two principal components does not really show any obvious cluster structure.

The assignment with $k=2$ has a slightly better average silhouette than the plot with $k=3$, but also a slightly worse within cluster sum of squares. Overall we would keep the model where $k=3$, as the division found by the algorithm seems reasonable and it does allow for more interesting insight.
