...
Now we are going to perform a model-based approach. This technique try to fit a mixture of gaussians into the data. The number of gaussians and the shape of them are need to be guessed. First, we need to compute the BIC for all the possible models, there is a function that perform this automatically.
...
As we can observe, the data does not look fitted very well. Nevertheless, from the previous plot, we can see that almost all types of mixtures behave similar when three components are present, so we can conclude that this kind of clustering does not behave well for this kind of data.
