...
plot(sil_kmeans3, col = "deepskyblue2")
```

Interesting to note is that most observations have a silhoutte close to 0, which means that they lie between two possible clusters.
...
```{r}
# Creo que no tiene mucho sentido poner estos plots porque los valores para las silhpouettes
# son tan pequeños que no se ve nada. Es mejor trabajar con la media de los valores y usar
# eso para comparar los metodos yo creo.
silhouette(pam_poke$clustering, dist(poke_numeric, method = "euclidean")) %>% plot()
```

So we can see that the average silhouette width is smaller than with kmeans. We can also try to use PAM clustering with manhattan distance instead of euclidean. In this case we get the following result:
...
Single Linkage selects the cluster by taking into consideration the minimum length to any observation of another cluster. That is, ignores the global distance of such cluster and measures single observations, producing huge clusters.

When applying Single Linkage to our data we see, using a dendogram (not useful when the number of observations is high), how the clustering procedure is done and how unbalanced the result is using the previous number of clusters $k=3$.
```{r}
poke_dists <- daisy(poke_numeric,metric="manhattan",stand=FALSE)
single <- hclust(poke_dists,method="single")

plot(single,main="Single linkage",cex=0.8)
rect.hclust(single,k=3,border="deepskyblue2")
```

As a result, when plotting the first two components of our PCA using these clusters the result is completely unbalanced.
```{r}
