...
```
We can see that the weights are all quite high in general. However, the variables related to the first pokemon have bigger values than the ones related to the second one. A possible interpretation of this is **TODO: AÑADIR INTERPRETACION**. 
...
# Factor Analysis
First we an explore the correlation of our numerical variables, ordering them to see whethere there are any groups among them:

```{r}
corrplot(cor(poke_numeric))
```

We can see quite clearly in the correlation plot that the variables related to pokemon 1 are completely uncorrelated with the ones related to pokemon 2. This makes sense, as they are completely separate pokemon.

We can now calculate the $M$ matrix for the first 5 principal components:
```{r}
...
```{r}
plot(1:p,M[,3],pch=19,col="deepskyblue2",xlab="",ylab="Weights",main="Weights for the third factor")
abline(h=0)
text(1:p,M[,3],labels=names(poke_numeric),pos = 1,col="firebrick2",cex=0.75)
```
```{r}
plot(1:p,M[,4],pch=19,col="deepskyblue2",xlab="",ylab="Weights",main="Weights for the fourth factor")
abline(h=0)
text(1:p,M[,4],labels=names(poke_numeric),pos = 1,col="firebrick2",cex=0.75)
```

```{r}
plot(1:p,M[,5],pch=19,col="deepskyblue2",xlab="",ylab="Weights",main="Weights for the fifth factor")
abline(h=0)
text(1:p,M[,5],labels=names(poke_numeric),pos = 1,col="firebrick2",cex=0.75)
```
...
So we can see that the variables that are better explained by the factors are `poke1_Sp.Def`, `poke2_Defense` and `Poke1_defense`. The ones that are worst explained are `poke2_HP`, `poke1_HP` and `poke2_Attack`.
